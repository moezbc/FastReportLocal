            ...
            <div>
                <h1 className="text-2xl font-bold text-white flex items-center gap-3">
                    <span className="p-2 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600">
                        <FiServer className="text-white" size={22} />
                    </span>
                    Paramétrage
                </h1>
                <p className="text-slate-400 mt-1">Configuration des serveurs de messagerie et de transfert</p>
            </div>

            {/* Section Tabs */}
            <div className="flex gap-1 p-1 glass-card rounded-xl">
                <button onClick={() => setSection('smtp')}
                    className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-all ${section === 'smtp' ? 'bg-gradient-to-r from-violet-500/20 to-cyan-500/20 text-white border border-white/10' : 'text-slate-400 hover:text-white hover:bg-white/5'
                        }`}>
                    <FiMail size={16} /> SMTP ({smtpList.length})
                </button>
                <button onClick={() => setSection('ftp')}
                    className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-all ${section === 'ftp' ? 'bg-gradient-to-r from-violet-500/20 to-cyan-500/20 text-white border border-white/10' : 'text-slate-400 hover:text-white hover:bg-white/5'
                        }`}>
                ...
                <div className="space-y-4">
                    ...
                    {showSmtp && (
                        <div className="glass-card p-6 space-y-4 border border-violet-500/20">
                            <h3 className="text-lg font-semibold text-white">{smtpEditId ? 'Modifier' : 'Nouveau'} serveur SMTP</h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-sm text-slate-400 mb-1 block">Nom *</label>
                                    <input type="text" value={smtpForm.name || ''} onChange={e => setSmtpForm({ ...smtpForm, name: e.target.value })} className="input-field w-full" /></div>
                                <div><label className="text-sm text-slate-400 mb-1 block">Serveur SMTP *</label>
                                ...
                                <div><label className="text-sm text-slate-400 mb-1 block">Port</label>
                                ...
                                <div><label className="text-sm text-slate-400 mb-1 block">Email expéditeur *</label>
                                ...
                                <div><label className="text-sm text-slate-400 mb-1 block">Utilisateur</label>
                                ...
                                <div><label className="text-sm text-slate-400 mb-1 block">Mot de passe</label>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button onClick={() => setShowSmtp(false)} className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-300 hover:bg-white/10">Annuler</button>
                            </div>
                        </div>
                    )}

                    {loading ? <div className="glass-card p-8 text-center text-slate-400">Chargement...</div> : smtpList.length === 0 ? (
                        <div className="glass-card p-12 text-center text-slate-400">Aucun serveur SMTP configuré</div>
                    ) : smtpList.map(s => (
                        <div key={s.id} className="space-y-2">
                            <div className="glass-card p-5 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div className="font-medium text-white flex items-center gap-2">{s.name} {s.is_default && <FiStar className="text-amber-400" size={14} />}</div>
                                        <div className="text-sm text-slate-400">{s.host}:{s.port} — {s.from_email}</div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={() => testSmtp(s.id)} disabled={testing === s.id} className="p-2 rounded-lg hover:bg-amber-500/10 text-slate-400 hover:text-amber-400" title="Tester la connexion SMTP">
                                        {testing === s.id ? <FiZap size={16} className="animate-pulse text-amber-400" /> : <FiZap size={16} />}
                                    </button>
                                    <button onClick={() => editSmtp(s)} className="p-2 rounded-lg hover:bg-white/5 text-slate-400 hover:text-white"><FiEdit2 size={16} /></button>
                                    <button onClick={() => delSmtp(s.id, s.name)} className="p-2 rounded-lg hover:bg-red-500/10 text-slate-400 hover:text-red-400"><FiTrash2 size={16} /></button>
                                </div>
                            </div>
            ...
            {/* ═══ FTP Section ═══ */}
            {
                section === 'ftp' && (
                    <div className="space-y-4">
                        ...
                        {showFtp && (
                            <div className="glass-card p-6 space-y-4 border border-violet-500/20">
                                <h3 className="text-lg font-semibold text-white">{ftpEditId ? 'Modifier' : 'Nouveau'} serveur FTP/SFTP</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="text-sm text-slate-400 mb-1 block">Nom *</label>
                                        <input type="text" value={ftpForm.name || ''} onChange={e => setFtpForm({ ...ftpForm, name: e.target.value })} className="input-field w-full" /></div>
                                    <div><label className="text-sm text-slate-400 mb-1 block">Protocole</label>
                                    ...
                                    <div><label className="text-sm text-slate-400 mb-1 block">Hôte *</label>
                                    ...
                                    <div className="col-span-2"><label className="text-sm text-slate-400 mb-1 block">Chemin distant</label>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button onClick={() => setShowFtp(false)} className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-300 hover:bg-white/10">Annuler</button>
                                </div>
                            </div>
                        )}

                        {loading ? <div className="glass-card p-8 text-center text-slate-400">Chargement...</div> : ftpList.length === 0 ? (
                            <div className="glass-card p-12 text-center text-slate-400">Aucun serveur FTP/SFTP configuré</div>
                        ) : ftpList.map(f => (
                            <div key={f.id} className="glass-card p-5 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div className="font-medium text-white flex items-center gap-2">{f.name} {f.is_default && <FiStar className="text-amber-400" size={14} />}
                                            <span className="px-2 py-0.5 rounded text-xs bg-white/5 text-slate-400">{f.protocol.toUpperCase()}</span>
                                        </div>
                                        <div className="text-sm text-slate-400">{f.host}:{f.port} — {f.remote_path}</div>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={() => editFtp(f)} className="p-2 rounded-lg hover:bg-white/5 text-slate-400 hover:text-white"><FiEdit2 size={16} /></button>
                                    <button onClick={() => delFtp(f.id, f.name)} className="p-2 rounded-lg hover:bg-red-500/10 text-slate-400 hover:text-red-400"><FiTrash2 size={16} /></button>
                                </div>
                            </div>
                        ))}
                    </div>
